...
import { 
  ...
  Filter, PieChart, Info, ImageIcon, Navigation, Bug, ChevronUp, ChevronDown, RotateCcw, Trash, Link as LinkIcon, Terminal, Activity
} from 'lucide-react';
...
// ★ バージョン定義
const VERSION = "Gen_v3.76-STABLE-RECOVERY";
...
const GourmetApp = () => {
  ...
  useEffect(() => {
    ...
    // パス: /artifacts/{appId}/public/data/stores
    ...
    const unsub = onSnapshot(storesCollection, (snap) => {
      ...
      setData(stores);
      setIsSyncing(false);
      addLog("SYNC_DATA", `${stores.length} items`);
    }, (err) => { 
      setIsSyncing(false);
  ...
  if (!authChecked) {
    return <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center p-10 text-center">
      ...
      <h2 className="text-3xl font-black text-white tracking-tighter mb-1">{VERSION}</h2>
      <p className="font-bold text-slate-500 uppercase tracking-widest text-xs">Authenticating...</p>
    </div>;
  }
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] text-slate-900 font-sans pb-24 sm:pb-0 relative overflow-x-hidden">
      ...
      <header className="sticky top-0 z-50 bg-white/95 backdrop-blur-xl border-b border-slate-200 h-16 md:h-20 flex items-center px-4 gap-3 shadow-sm">
        ...
        <div className={`flex items-center gap-1 px-3 py-2 rounded-xl border shadow-inner shrink-0 min-w-[75px] justify-center ${user ? 'bg-orange-50 border-orange-200' : 'bg-slate-100 border-slate-200'}`}>
      ...
      <main className="max-w-7xl mx-auto p-6 md:p-10 min-h-screen">
        <div className="mb-12 text-center animate-in fade-in duration-1000">
           ...
           <div className={`inline-block px-8 py-2.5 rounded-full font-black text-[11px] uppercase tracking-widest shadow-xl ${user ? 'bg-orange-500 text-white' : 'bg-slate-200 text-slate-500'}`}>
             {user ? 'Automatically Connected to Global Hub' : 'System Standalone Mode'}
           </div>
        </div>
        ...
        <div className="max-w-4xl mx-auto mb-16 bg-slate-900 rounded-[2.5rem] overflow-hidden shadow-2xl border border-slate-800">
          <div className="bg-slate-800 px-8 py-5 flex items-center justify-between">
            <div className="flex items-center gap-3"><Activity className="text-orange-500 animate-pulse" size={20} /><span className="text-[10px] font-black text-white uppercase tracking-widest">Live Sync Dashboard</span></div>
            <button onClick={() => setReconnectTrigger(t => t + 1)} className="px-4 py-1.5 bg-slate-700 hover:bg-slate-600 rounded-full transition-all active:scale-95"><RotateCcw size={12} className="text-orange-400 inline mr-2" /><span className="text-[9px] text-white font-black uppercase">Reconnect</span></button>
          </div>
          <div className="p-8 grid grid-cols-2 sm:grid-cols-4 gap-4 text-[9px] font-black uppercase tracking-tighter text-center">
            <div className="bg-slate-800/50 p-4 rounded-2xl border border-white/5"><p className="text-slate-500 mb-1">Session UID</p><p className="text-blue-400 truncate">{user?.uid.slice(0,12) || '---'}</p></div>
            <div className="bg-slate-800/50 p-4 rounded-2xl border border-white/5"><p className="text-slate-500 mb-1">Local Data</p><p className="text-green-400 text-lg">{data.length}</p></div>
            <div className="bg-black/40 p-4 rounded-2xl border border-white/5 col-span-2 h-20 overflow-y-auto scrollbar-hide text-left">
               {logs.map((l, i) => ( <div key={i} className="flex gap-2 mb-1 opacity-70 border-b border-white/5 pb-1"><span className="text-slate-600">{l.time}</span><span className="text-orange-500">{l.event}</span></div> ))}
            </div>
          </div>
        </div>
        ...
        {data.length === 0 ? (
          <div className="max-w-4xl mx-auto py-20 text-center bg-white p-16 rounded-[4rem] shadow-2xl border border-slate-100 animate-in zoom-in duration-500">
              ...
              <h2 className="text-4xl font-black mb-6 text-slate-800 tracking-tight italic uppercase leading-none">Wait for Import</h2>
              <p className="text-slate-400 mb-12 font-bold max-w-sm mx-auto text-sm leading-relaxed">Windowsでエクセルを取り込んでください。自動的にここに現れます。</p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-w-lg mx-auto">
                <button onClick={() => saveDataToCloud([{NO:1,店舗名:"サンプル店",カテゴリ:"和食",都道府県:"東京都",住所:"銀座",isFavorite:true}])} className="py-6 bg-orange-500 text-white rounded-[2rem] font-black shadow-xl hover:bg-orange-600 transition-all active:scale-95 text-xl tracking-widest uppercase">Sample</button>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                        {stores.map(store => (
                          <div key={store.id} className="bg-white rounded-[4rem] shadow-sm border border-slate-100 hover:border-orange-200 hover:shadow-2xl transition-all duration-700 flex flex-col group relative">
                            ...
                            <div className="p-10 flex-1 flex flex-col justify-between gap-10"><p className="line-clamp-2 leading-relaxed italic text-slate-500 font-bold text-sm">{store.住所 || "No address provided."}</p>
      ...
      <div className="fixed bottom-10 left-1/2 -translate-x-1/2 sm:hidden z-40 flex items-center gap-4"><label className="bg-slate-900 text-white px-8 py-5 rounded-[2rem] font-black shadow-2xl active:scale-110 transition-all flex items-center gap-3 text-sm uppercase tracking-widest shadow-slate-400"><Upload size={20}/> Import Excel<input type="file" className="hidden" accept=".csv, .xlsx" onChange={handleFileUpload} /></label></div>
    </div>
  );
};
