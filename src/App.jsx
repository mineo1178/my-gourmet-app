...
const VERSION = "Gen_v3.72-STABLE-SYNC";
    ...
    if (this.state.hasError) {
      return (
        <div className="min-h-screen bg-slate-900 text-white p-8 font-mono flex flex-col items-center justify-center text-center text-xs">
          ...
          <button onClick={() => window.location.reload()} className="mt-6 px-6 py-3 bg-white text-slate-900 rounded-xl font-black uppercase shadow-2xl transition-all">Reload System</button>
        </div>
      );
    }
...
const GourmetApp = () => {
  ...
  // --- ★ 修正: 認証 Rule 3 (起動時に自動実行) ---
  useEffect(() => {
    const initAuth = async () => {
      try {
        addLog("AUTH_INIT", "Starting...");
  ...
  // --- ★ 修正: 同期 Rule 1 & Rule 3 ---
  useEffect(() => {
    if (!user) return; // 認証が済むまで待機

    setIsSyncing(true);
    addLog("SYNC_START", "Connecting to public data...");
    
    // パス構造: /artifacts/{appId}/public/data/stores
    ...
    const unsub = onSnapshot(storesCollection, (snap) => {
      ...
      setIsSyncing(false);
      addLog("SYNC_PERMISSION_ERROR", err.code);
      console.error("Firestore Permission Issue:", err);
    });
  ...
  const deleteData = async (id) => {
    ...
    try { 
      const docRef = doc(db, 'artifacts', appId, 'public', 'data', 'stores', id);
  ...
  const toggleFavorite = async (store) => {
    if (!user) return;
    try {
      const docRef = doc(db, 'artifacts', appId, 'public', 'data', 'stores', store.id);
  ...
  if (!authChecked) {
    return <div className="min-h-screen bg-white flex flex-col items-center justify-center p-10 text-center">
      ...
      <p className="font-bold text-slate-400 uppercase tracking-widest text-xs">Connecting to Cloud...</p>
    </div>;
  }

  // --- UI: Debug monitor on Dashboard ---
  const Monitor = () => (
    <div className="max-w-4xl mx-auto mb-16 bg-slate-900 rounded-[2.5rem] overflow-hidden shadow-2xl border border-slate-800 animate-in fade-in duration-1000">
      <div className="bg-slate-800 px-8 py-5 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Terminal className="text-orange-500" size={20} />
          <span className="text-[10px] font-black text-white uppercase tracking-widest italic">Live Network Monitor</span>
        </div>
        <button onClick={() => setReconnectTrigger(t => t + 1)} className="flex items-center gap-2 px-4 py-1.5 bg-slate-700 hover:bg-slate-600 rounded-full transition-all active:scale-95">
           <RotateCcw size={12} className="text-orange-400" />
           <span className="text-[9px] text-white font-black uppercase">Force Reconnect</span>
        </button>
      </div>
      <div className="p-8 space-y-6">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-[9px] font-black uppercase tracking-tighter">
          <div className="bg-slate-800/50 p-4 rounded-2xl border border-white/5 shadow-inner">
            ...
            <p className="text-blue-400 truncate font-mono">{user?.uid || 'NONE'}</p>
          </div>
          <div className="bg-slate-800/50 p-4 rounded-2xl border border-white/5 shadow-inner">
            <p className="text-slate-500 mb-1">Items</p>
            <p className="text-green-400 text-lg">{data.length}</p>
          </div>
          <div className="bg-slate-800/50 p-4 rounded-2xl border border-white/5 shadow-inner">
            <p className="text-slate-500 mb-1">Sync</p>
            <p className={isSyncing ? 'text-orange-500 animate-pulse' : 'text-slate-400'}>{isSyncing ? 'ACTIVE' : 'STABLE'}</p>
          </div>
          <div className="bg-slate-800/50 p-4 rounded-2xl border border-white/5 shadow-inner">
            <p className="text-slate-500 mb-1">Domain</p>
            <p className="text-slate-400 truncate">public/data/stores</p>
          </div>
        </div>
        
        <div className="bg-black/40 rounded-3xl p-6 h-40 overflow-y-auto scrollbar-hide font-mono text-[10px] border border-white/5">
           {logs.map((l, i) => (
             <div key={`debug-${i}`} className={`flex gap-4 mb-2 opacity-80 border-b border-white/5 pb-2 last:border-0 ${l.event.includes('ERROR') ? 'text-rose-400' : ''}`}>
               <span className="text-slate-600 shrink-0">{l.time}</span>
               <span className="text-orange-500 font-bold shrink-0">{l.event}</span>
    ...
    <div className="min-h-screen bg-[#F8FAFC] text-slate-900 font-sans pb-24 sm:pb-0 relative overflow-x-hidden">
      ...
      <header className="sticky top-0 z-50 bg-white/95 backdrop-blur-xl border-b border-slate-200 h-16 md:h-20 flex items-center px-4 gap-3 shadow-sm">
        ...
        <div className={`flex items-center gap-1.5 px-3 py-2 rounded-xl border shadow-inner shrink-0 min-w-[75px] justify-center ${user ? 'bg-orange-50 border-orange-200' : 'bg-slate-100 border-slate-200'}`}>
          ...
          <span className={`text-[10px] font-black uppercase ${user ? 'text-orange-700' : 'text-slate-500'}`}>
            {user ? 'Cloud' : 'Offline'}
          </span>
        </div>
      ...
      <main className="max-w-7xl mx-auto p-6 md:p-10 min-h-screen">
        <div className="mb-12 text-center animate-in fade-in duration-1000">
           <p className="text-[10px] font-black text-slate-300 uppercase tracking-[0.6em] mb-3 leading-none tracking-tighter">Secure Synchronization Engine</p>
        ...
        {data.length === 0 ? (
          <div className="max-w-4xl mx-auto py-24 text-center bg-white p-16 rounded-[4rem] shadow-2xl border border-slate-100 animate-in zoom-in duration-500">
              ...
              <p className="text-slate-400 mb-12 font-bold max-w-sm mx-auto text-sm leading-relaxed">
                エクセルファイルを選択して読み込んでください。<br/>
                Windowsで取り込めばiPhoneにも自動反映されます。
              </p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-w-lg mx-auto">
                <button onClick={() => saveDataToCloud([{NO:1,店舗名:"サンプル銘店",カテゴリ:"和食",都道府県:"東京都",住所:"銀座",isFavorite:true}])} className="py-6 bg-orange-500 text-white rounded-[2rem] font-black shadow-xl hover:bg-orange-600 transition-all active:scale-95 text-xl italic tracking-widest uppercase">Sample</button>
                          ...
                          <div key={`card-${store.id}`} className="bg-white rounded-[4rem] shadow-sm border border-slate-100 hover:border-orange-200 hover:shadow-2xl transition-all duration-700 flex flex-col group relative">
                            ...
                            <div className="p-10 flex-1 flex flex-col justify-between gap-10">
                              ...
                              <div className="flex gap-4 pt-6 border-t border-slate-50">
                                ...
                                <button onClick={() => deleteData(store.id)} className="p-4 bg-slate-50 text-slate-400 hover:text-red-600 hover:bg-white rounded-[1.5rem] transition-all shadow-inner"><Trash2 size={24}/></button>
                              </div>
                            </div>
                          </div>
                        ))}
