...
// ★ バージョン（要望どおり）
const VERSION = "GTP_v3.70-PUBLIC-SYNC";
...
const GourmetApp = () => {
  ...
  // ★ 固定共有パス（全員共有 / セキュリティ捨て）
  // Firestore Web SDKは「セグメント指定」が正道
  const firestorePathSegments = useMemo(() => ['artifacts', appId, 'public', 'data', 'stores'], []);
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snap) => {
      ...
      if ((err.code || "").toLowerCase() === "permission-denied") {
        addLog("HINT", "Firestore Rulesがreadを拒否。環境(設定)側が原因。");
      }
    });
  ...
  const handleFileUpload = (event) => {
    ...
    if (!file || !window.XLSX) {
      alert("XLSXライブラリ未準備です。少し待ってから再度お試しください。");
      return;
    }
  ...
  useEffect(() => {
    ...
    script.async = true;
    script.onload = () => addLog("XLSX_READY");
        ...
        {data.length === 0 ? (
          <div className="max-w-3xl mx-auto py-16 text-center bg-white p-10 rounded-[3rem] shadow-xl border border-slate-100">
            ...
            <p className="text-sm text-slate-400 font-bold mb-8 leading-relaxed">
              WindowsでExcelを取り込むと、iPhoneにも自動で反映されます。
            </p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-lg mx-auto">
              ...
              <label className="py-4 border-2 border-slate-200 text-slate-600 rounded-2xl font-black cursor-pointer hover:bg-slate-50 transition-all text-lg flex items-center justify-center gap-2 italic tracking-widest uppercase">
                IMPORT
                <input type="file" className="hidden" accept=".csv, .xlsx" onChange={handleFileUpload} />
              </label>
            </div>
