...
const GourmetApp = () => {
  ...
  const SyncSettingsCard = () => (
    <div className="max-w-4xl mx-auto mb-12 bg-white rounded-[3rem] p-6 sm:p-10 border border-slate-100 shadow-xl animate-in fade-in duration-700">
      <div className="flex flex-col md:flex-row items-center gap-6 md:gap-10">
        <div className="flex-1 space-y-3">
          <div className="flex items-center gap-3">
            <div className="bg-orange-500 p-2 rounded-xl text-white shadow-lg"><Key size={20} /></div>
            <h3 className="font-black text-2xl italic">同期設定 (Sync Settings)</h3>
          </div>
          <p className="text-sm text-slate-400 font-bold leading-relaxed">
            WindowsとiPhoneで同じ「合言葉」を入力してください。<br className="hidden sm:block"/>
            入力すると「Cloud」に切り替わり、データが同期されます。
          </p>
        </div>
        <div className="w-full md:w-auto flex flex-col gap-4 min-w-[300px]">
          ...
          />
          {shareKey && (
            <button onClick={() => { if(window.confirm("Windows側のリストをクラウド(合言葉)へ送信しますか？")) saveData(data); }} className="w-full py-4 bg-orange-600 text-white rounded-2xl font-black hover:bg-orange-700 transition-all text-xs shadow-lg flex items-center justify-center gap-2 uppercase tracking-tighter active:scale-95">
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] text-slate-900 font-sans selection:bg-orange-100 relative overflow-x-hidden pb-20 sm:pb-0">
      ...
      {/* 診断ログ用 (Bugアイコン) */}
      <button onClick={() => setIsDebugOpen(!isDebugOpen)} className="fixed bottom-4 right-4 z-[100] p-3 bg-slate-900 text-white rounded-full shadow-2xl opacity-40 hover:opacity-100 transition-all">
        <Bug size={18} />
      </button>
      ...
      {isDebugOpen && (
        <div className="fixed bottom-0 right-0 z-[110] w-full sm:w-96 h-[60vh] bg-slate-900 text-[9px] text-slate-300 font-mono p-4 border-t border-white/20 shadow-2xl overflow-y-auto scrollbar-hide">
          <div className="flex justify-between items-center mb-4 border-b border-white/10 pb-2">
            <span className="font-bold text-orange-500 uppercase tracking-widest">SYSTEM LOG</span>
      ...
      <header className="sticky top-0 z-50 bg-white/90 backdrop-blur-xl border-b border-slate-200 h-16 md:h-20 flex items-center px-4 gap-4">
        ...
        <div className="flex-1 relative group max-w-xl">
          ...
          <input type="text" placeholder="店名検索..." className="w-full pl-11 pr-4 py-2.5 bg-slate-100/80 border-none rounded-2xl text-sm outline-none focus:bg-white focus:ring-4 focus:ring-orange-500/5 transition-all font-bold" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
        </div>
        
        <div className={`flex items-center gap-1.5 px-3 py-2 rounded-xl border shadow-inner shrink-0 ${firestoreCollectionPath ? 'bg-orange-50 border-orange-200' : 'bg-slate-100 border-slate-200'}`}>
        ...
        <div className="flex items-center gap-2 shrink-0">
           ...
           {(!user || user.isAnonymous) && (
             <button onClick={startLogin} className="p-2.5 bg-white border border-slate-200 text-slate-400 rounded-2xl hover:bg-slate-50 transition-all" title="Googleログイン"><LinkIcon size={20} /></button>
           )}
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto p-4 md:p-8 min-h-screen">
        ...
        <div className="mb-12 text-center animate-in fade-in duration-1000">
           <p className="text-[10px] font-black text-slate-300 uppercase tracking-[0.5em] mb-2 leading-none tracking-tighter">AUTHENTICATED ENVIRONMENT</p>
           <h2 className="text-6xl sm:text-9xl font-black text-slate-900 italic tracking-tighter leading-none mb-4">{VERSION}</h2>
           <div className="flex justify-center gap-3">
             <div className={`px-4 py-2 rounded-full font-black text-[11px] uppercase tracking-widest ${firestoreCollectionPath ? 'bg-orange-500 text-white shadow-xl' : 'bg-slate-200 text-slate-500'}`}>
               {firestoreCollectionPath ? 'Cloud Synchronized' : 'Local Standing'}
             </div>
           </div>
        </div>

        {/* 同期設定カード (Windowsでも見える) */}
        <SyncSettingsCard />

        {data.length === 0 ? (
          <div className="max-w-4xl mx-auto py-16 text-center">
              <div className="bg-white p-12 rounded-[4rem] shadow-xl border border-slate-100 animate-in zoom-in duration-500 flex flex-col items-center">
                <Database className="text-orange-500 mb-8 opacity-20" size={100} />
                ...
                <p className="text-slate-400 mb-10 font-bold max-w-sm mx-auto text-sm leading-relaxed">
                  Excelからデータを取り込むか、<br/>
                  上の入力欄に「合言葉」を入れてクラウド同期を開始してください。
                </p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full max-w-lg">
                  <button onClick={() => saveData([{NO:1,店舗名:"サンプル店",カテゴリ:"飲食店",都道府県:"東京都",住所:"銀座",isFavorite:true}])} className="py-6 bg-orange-500 text-white rounded-3xl font-black shadow-xl hover:bg-orange-600 transition-all active:scale-95 text-lg italic tracking-widest uppercase">Sample</button>
                  <label className="py-6 border-2 border-slate-200 text-slate-600 rounded-3xl font-black cursor-pointer hover:bg-slate-50 transition-all text-lg flex items-center justify-center gap-2 italic tracking-widest uppercase">Import<input type="file" className="hidden" accept=".csv, .xlsx" onChange={handleFileUpload} /></label>
                </div>
              </div>
          </div>
        ) : (
            ...
            {activeTab === 'map' ? (
              ...
              <div className="flex flex-col lg:flex-row gap-10 animate-in slide-in-from-bottom-6">
                <aside className="lg:w-80 shrink-0 hidden lg:block">
                   <div className="bg-white p-8 rounded-[3rem] border border-slate-200 shadow-sm sticky top-44 space-y-8 max-h-[65vh] overflow-y-auto scrollbar-hide">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2 border-b border-slate-50 pb-4 italic tracking-[0.2em]"><ArrowDown size={14} className="text-orange-500" /> Genre Jump</p>
                ...
                <div className="flex-1 space-y-24 min-w-0 pb-32">
                  {groupedData.length === 0 ? <div className="bg-white p-24 rounded-[4rem] text-center text-slate-300 font-black italic shadow-inner text-xl">表示できるデータがありません</div> : groupedData.map(([category, stores]) => (
                    <div key={category} id={`category-section-${category}`} className="space-y-10 scroll-mt-44">
      ...
      <footer className="w-full py-16 text-center text-[10px] font-black text-slate-300 uppercase tracking-[0.5em] bg-white border-t sm:hidden mb-4 leading-relaxed px-10">
        VER {VERSION} | SHARED CLOUD SYSTEM<br/>
        SYNCHRONIZED BY GOURMET MASTER
      </footer>
    </div>
  );
};
