...
import { 
  ...
  Filter, PieChart, Info, ImageIcon, Navigation, Bug, ChevronUp, ChevronDown, RotateCcw, Trash, Link as LinkIcon, Terminal
} from 'lucide-react';
...
// ★ バージョン定義
const VERSION = "Gen_v3.70-DEBUG-INTEGRATED";
...
const GourmetApp = () => {
  ...
  // 自動匿名ログイン
  useEffect(() => {
    if (!canUseCloud) { setAuthChecked(true); addLog("INIT_OFFLINE"); return; }
    const unsub = onAuthStateChanged(auth, async (u) => {
      if (u) {
        setUser(u);
        setAuthChecked(true);
        addLog("AUTH_USER_READY", u.uid);
      } else {
        try {
  ...
  useEffect(() => {
    ...
    setIsSyncing(true);
    addLog("SYNC_SUBSCRIBING", firestorePath);
    const q = collection(db, firestorePath);
    const unsub = onSnapshot(q, (snap) => {
      const stores = snap.docs.map(doc => ({ id: doc.id, ...doc.data() }));
      setData(stores);
      setIsSyncing(false);
      addLog("SYNC_DATA_RECEIVED", `${stores.length} items`);
    }, (err) => { 
      setIsSyncing(false);
      addLog("SYNC_CRITICAL_ERR", err.code);
      console.error(err);
    });
    return () => unsub();
  }, [user]);
  ...
  const deleteData = async (id) => {
    ...
    if (canUseCloud) {
      try { await deleteDoc(doc(db, firestorePath, id)); addLog("DELETE_OK"); } catch(e) { addLog("DELETE_FAIL", e.code); }
    }
  };
  ...
  if (!authChecked) {
    return <div className="min-h-screen bg-white flex flex-col items-center justify-center p-10 text-center">
      ...
      <p className="font-bold text-slate-400 uppercase tracking-widest text-xs">Authenticating Cloud Session...</p>
    </div>;
  }

  // --- UI Parts: Debug Log Monitor ---
  const DebugMonitor = () => (
    <div className="max-w-4xl mx-auto mb-16 bg-slate-900 rounded-[2.5rem] overflow-hidden shadow-2xl border border-slate-800 animate-in fade-in duration-1000">
      <div className="bg-slate-800 px-8 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className="text-[10px] font-black text-white uppercase tracking-widest">Live Sync Monitor</span>
        </div>
        <div className="flex items-center gap-2">
           <span className={`w-2 h-2 rounded-full animate-pulse ${canUseCloud ? 'bg-green-500' : 'bg-rose-500'}`} />
           <span className="text-[9px] text-slate-400 font-bold uppercase">{canUseCloud ? 'Connected' : 'Offline'}</span>
        </div>
      </div>
      <div className="p-8 space-y-4">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-[9px] font-black uppercase tracking-tighter">
          <div className="bg-slate-800/50 p-3 rounded-xl border border-white/5">
            <p className="text-slate-500 mb-1">Session UID</p>
            <p className="text-blue-400 truncate">{user?.uid || 'N/A'}</p>
          </div>
          <div className="bg-slate-800/50 p-3 rounded-xl border border-white/5">
            <p className="text-slate-500 mb-1">Sync Path</p>
            <p className="text-orange-400 truncate">/public/data/stores</p>
          </div>
          <div className="bg-slate-800/50 p-3 rounded-xl border border-white/5">
            <p className="text-slate-500 mb-1">Local Data</p>
            <p className="text-green-400">{data.length} items</p>
          </div>
          <div className="bg-slate-800/50 p-3 rounded-xl border border-white/5">
            <p className="text-slate-500 mb-1">Sync Status</p>
            <p className={isSyncing ? 'text-orange-500' : 'text-slate-400'}>{isSyncing ? 'SYNCING...' : 'IDLE'}</p>
          </div>
        </div>
        
        <div className="bg-black/40 rounded-2xl p-4 h-32 overflow-y-auto scrollbar-hide font-mono text-[9px] border border-white/5">
           {logs.slice(0, 10).map((l, i) => (
             <div key={`debug-${i}`} className="flex gap-3 mb-1.5 opacity-80 border-b border-white/5 pb-1 last:border-0">
    ...
    <div className="min-h-screen bg-[#F8FAFC] text-slate-900 font-sans pb-24 sm:pb-0 relative overflow-x-hidden">
      ...
      <main className="max-w-7xl mx-auto p-6 md:p-10 min-h-screen">
        ...
        <div className="mb-12 text-center animate-in fade-in duration-1000">
           <p className="text-[10px] font-black text-slate-300 uppercase tracking-[0.6em] mb-3 leading-none tracking-tighter">Unified Synchronization System</p>
        ...
        {/* デバッグモニター: ここにログが表示されます */}
        <DebugMonitor />

        {data.length === 0 ? (
          <div className="max-w-4xl mx-auto py-16 text-center bg-white p-16 rounded-[4rem] shadow-2xl border border-slate-100 animate-in zoom-in duration-500">
              <Database className="mx-auto text-orange-500 mb-8 opacity-20" size={100} />
              <h2 className="text-4xl font-black mb-6 text-slate-800 tracking-tight italic uppercase">Wait for Windows Import</h2>
              <p className="text-slate-400 mb-12 font-bold max-w-sm mx-auto text-sm leading-relaxed">
                Windowsでエクセルを取り込むと、このiPhoneにも自動表示されます。<br/>
                同期されない場合は、上のモニターログを確認してください。
              </p>
                  ...
                  {groupedData.length === 0 ? <div className="bg-white p-32 rounded-[5rem] text-center text-slate-300 font-black italic shadow-inner text-2xl uppercase tracking-widest">No Items Found</div> : groupedData.map(([category, stores]) => (
                    <div key={`section-${category}`} id={`category-section-${category}`} className="space-y-12 scroll-mt-48 animate-in fade-in duration-1000">
                      ...
                      <div className={viewMode === 'detail' ? "grid grid-cols-1 md:grid-cols-2 gap-12" : "space-y-6"}>
                        {stores.map(store => (
                          <div key={`card-${store.id}`} className="bg-white rounded-[4rem] shadow-sm border border-slate-200/40 overflow-hidden hover:shadow-2xl transition-all duration-700 flex flex-col group relative">
